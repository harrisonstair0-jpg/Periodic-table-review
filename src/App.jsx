...
function App() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#090b10] text-slate-100">
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-[1800px] flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
        <Header
    ...
    <header className="flex flex-col gap-4 border-b border-white/10 pb-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          <div className="mb-2 flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.24em] text-teal-200/80">
            <Sparkles className="h-4 w-4" />
            Element explorer
          </div>
          <h1 className="font-display text-4xl font-black tracking-normal text-white sm:text-5xl lg:text-6xl">
            Periodic Table
          </h1>
          <p className="mt-3 max-w-2xl text-base leading-7 text-slate-300">
            Explore the chemical elements in a glowing table. Search, filter,
            and click any tile to learn the basics, uses, fun facts, and videos.
          </p>
        </div>

        <div className="grid gap-3 sm:grid-cols-[minmax(240px,360px)_auto] lg:items-center">
          <label className="relative block">
            ...
            <input
              ...
              className="h-12 w-full rounded-md border border-white/15 bg-white/[0.07] pl-10 pr-4 text-sm text-white outline-none transition focus:border-teal-300 focus:bg-white/[0.10] focus:ring-2 focus:ring-teal-300/30"
            />
          </label>
          <div className="flex h-12 items-center justify-center rounded-md border border-white/10 bg-white/[0.06] px-4 text-sm font-semibold text-slate-200">
            {resultCount} tiles
          </div>
        </div>
      </div>
        ...
        {[allCategoriesLabel, ...categories].map((category) => {
          ...
          return (
            <button
              ...
              className="inline-flex h-10 items-center gap-2 rounded-md border px-3 text-sm font-semibold transition hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-white/35"
              style={{
...
function PeriodicTable({ elementsList, selectedId, onSelect }) {
  ...
  if (elementsList.length === 0) {
    return (
      <section className="flex min-h-[460px] items-center justify-center rounded-md border border-dashed border-white/20 bg-white/[0.04] px-6 text-center">
        <div>
  ...
  return (
    <section aria-label="Periodic table of elements" className="min-w-0">
      <div className="table-scroll overflow-x-auto rounded-md border border-white/10 bg-black/20 p-3 shadow-2xl shadow-black/40 backdrop-blur">
        <div className="periodic-grid grid gap-2 lg:gap-2.5">
...
function ElementTile({ element, selected, onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group relative flex aspect-square min-h-[72px] flex-col overflow-hidden rounded-md border p-1.5 text-left transition duration-300 hover:-translate-y-1 hover:shadow-neon focus:outline-none focus:ring-2 focus:ring-white/40 sm:min-h-[76px]"
      style={{
    ...
    >
      ...
      <span className="mt-1.5 line-clamp-2 min-h-[1.6rem] text-[11px] font-bold leading-[1.1] text-slate-100">
        {element.name}
      </span>
      <span className="mt-auto flex items-center gap-1 text-[10px] font-medium text-slate-300">
        <CalendarDays className="h-3 w-3" />
        Mass {element.atomicMass}
      </span>
      <span className="mt-0.5 line-clamp-2 text-[9px] leading-tight text-slate-400">
        {element.category}
      </span>

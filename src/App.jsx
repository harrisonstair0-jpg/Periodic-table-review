...
function App() {
  ...
  return (
    <main className="h-screen overflow-hidden bg-[#090b10] text-slate-100">
      ...
      <section className="relative z-10 flex h-screen w-full flex-col gap-2 overflow-hidden px-2 py-2">
        <Header
...
function Header({
  ...
  return (
    <header className="z-20 flex shrink-0 flex-col gap-1.5 rounded-md border border-white/10 bg-black/55 px-2 py-1.5 shadow-2xl shadow-black/40 backdrop-blur-xl lg:flex-row lg:items-center">
      <div className="flex min-w-0 flex-col gap-1.5 lg:flex-1">
        <div className="flex flex-col gap-1.5 lg:flex-row lg:items-center">
          <div className="flex min-w-0 items-center gap-3">
            <div className="hidden items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-teal-200/80 md:flex">
          ...
          <div className="grid gap-2 sm:grid-cols-[minmax(220px,340px)_auto] lg:ml-auto lg:items-center">
            <label className="relative block">
              ...
              <input
                ...
                className="h-7 w-full rounded-md border border-white/15 bg-white/[0.07] pl-9 pr-3 text-sm text-white outline-none transition focus:border-teal-300 focus:bg-white/[0.10] focus:ring-2 focus:ring-teal-300/30"
              />
            </label>
            <div className="flex h-7 items-center justify-center rounded-md border border-white/10 bg-white/[0.06] px-3 text-xs font-semibold text-slate-200">
              {resultCount} tiles
            </div>
          </div>
        </div>
      ...
      <nav aria-label="Category filters" className="table-scroll flex min-w-0 flex-1 gap-1.5 overflow-x-auto lg:max-w-[48vw]">
        {[allCategoriesLabel, ...categories].map((category) => {
          ...
          return (
            <button
              ...
              className="inline-flex h-6 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-md border px-2 text-xs font-semibold transition hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-white/35"
              style={{
...
function PeriodicTable({ elementsList, selectedId, onSelect }) {
  ...
  return (
    <section aria-label="Periodic table of elements" className="min-h-0 min-w-0 flex-1">
      <div className="table-scroll h-full w-full overflow-auto rounded-md border border-white/10 bg-black/10 p-1 shadow-2xl shadow-black/40 backdrop-blur">
        <div className="periodic-grid grid gap-2">
...
function ElementTile({ element, selected, onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group relative flex h-full min-h-[52px] w-full flex-col overflow-hidden rounded-md border p-1 text-left transition duration-300 hover:-translate-y-1 hover:shadow-neon focus:outline-none focus:ring-2 focus:ring-white/40"
      style={{
